...
async def start_improve_product(
    ...
    await state.set_state(ImproveProductStates.waiting_for_text)
    await message.answer(
        "✨ <b>Улучшение карточки товара</b>\n\n"
        "Вставьте текущее описание товара (можно скопировать прямо с маркетплейса):",
        reply_markup=cancel_keyboard(),
    )


@router.message(ImproveProductStates.waiting_for_text)
async def process_improve_text(
    message: Message, state: FSMContext, session: AsyncSession
) -> None:
    if not message.text:
        await message.answer("Пожалуйста, отправьте текстовое описание товара.")
        return
    ...
    await message.answer("⏳ Улучшаю карточку товара, это займёт 10–30 секунд…")

    generation_service = GenerationService(user_repo)
    try:
        card = await generation_service.improve_product(
            user=user, existing_text=message.text.strip()
        )
    ...
    await message.answer(
        f"✅ Списано 2 генерации. Остаток: <b>{user.generation_balance}</b>."
    )
