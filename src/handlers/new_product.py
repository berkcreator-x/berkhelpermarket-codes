...
from src.ai import GenerationService, GenerationServiceError, InsufficientBalanceError
...
async def start_new_product(message: Message, state: FSMContext, session: AsyncSession) -> None:
...
@router.message(NewProductStates.waiting_for_name)
async def process_name(message: Message, state: FSMContext) -> None:
    if not message.text:
        await message.answer("Пожалуйста, введите текст.")
        return
    await state.update_data(name=message.text.strip())
    await state.set_state(NewProductStates.waiting_for_category)
    await message.answer(
...
@router.message(NewProductStates.waiting_for_category)
async def process_category(message: Message, state: FSMContext) -> None:
    if not message.text:
        await message.answer("Пожалуйста, введите текст.")
        return
    await state.update_data(category=message.text.strip())
    await state.set_state(NewProductStates.waiting_for_features)
    await message.answer(
...
@router.message(NewProductStates.waiting_for_features)
async def process_features(message: Message, state: FSMContext) -> None:
    if not message.text:
        await message.answer("Пожалуйста, введите текст.")
        return
...
@router.message(NewProductStates.waiting_for_audience)
async def process_audience(message: Message, state: FSMContext, session: AsyncSession) -> None:
    if not message.text:
        await message.answer("Пожалуйста, введите текст.")
        return
    ...
    try:
        ...
        await message.answer(
            "⚠️ Недостаточно генераций. Пополните баланс в разделе «💳 Генерации».",
    ...
    await message.answer(card.to_message(), reply_markup=main_menu_keyboard(is_admin=is_admin))
    await message.answer(
        f"✅ Списана 1 генерация. Остаток: <b>{user.generation_balance}</b>."
    )
