...
для нейросети, генерирующей изображения (image editing model).

Пользователь прислал реальное фото своего товара и хочет
получить 5 профессиональных продающих изображений для
карточки на маркетплейсе (Wildberries/Ozon), на основе
ЭТОГО ЖЕ реального товара.
...
- явно указывать, что нужно СОХРАНИТЬ реальный товар с фото
  без изменений (его форму, цвет, детали), меняя только
  фон, окружение, композицию и добавляя элементы дизайна;
- учитывать пожелания пользователя по стилю;
- быть самодостаточным и подробным (это не превью, а
  полноценное ТЗ для генерации).

Роли 5 изображений строго в этом порядке:

1. Главная обложка — товар крупным планом, эффектный фон
2. Акцент на преимуществах товара
3. Демонстрация характеристик/деталей товара
4. Товар в контексте использования
5. Финальный призывающий к покупке кадр
